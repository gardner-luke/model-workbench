...
test('smoke test - app loads and displays registry page', async ({ page }) => {
  await page.goto('/');

  await expect(page.getByRole('heading', { name: 'Model Workbench' })).toBeVisible();
  await expect(page.getByText('Endpoint Registry')).toBeVisible();
