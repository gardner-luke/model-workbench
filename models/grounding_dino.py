...
# MAGIC
# MAGIC Registers a PyFunc wrapper around `IDEA-Research/grounding-dino-base` for detecting
# MAGIC objects described by any free-text prompt.
...
UC_NAME = f"{UC_CATALOG}.{UC_SCHEMA}.grounding_dino"
...
_in = pd.DataFrame([{"image": _tiny, "text_prompt": "red square.", "threshold": 0.3}])
...
payload = {
    ...
    "config": {
        ...
        "traffic_config": {"routes": [{"served_model_name": "grounding-dino", "traffic_percentage": 100}]},
    },
}
resp = requests.post(f"{HOST}/api/2.0/serving-endpoints", headers=HEADERS, json=payload)
if resp.status_code == 200:
    print(f"✓ Endpoint 'grounding-dino' created")
elif "already exists" in resp.text.lower():
    print(f"⏭ Endpoint 'grounding-dino' already exists")
else:
